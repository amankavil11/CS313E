...
class Graph (object):
    ...
    def hiking(self):
        #Eulerian Path of undirected graph
        lstOfAdjEdges = []
        
        #find number of edges from each vertex
        for i in range(len(self.Vertices)):
            lstOfAdjEdges.append(sum(self.adjMat[i]))
            
            oddNumEdges = 0
        for i in range(len(self.Vertices)-1,-1,-1):
            if lstOfAdjEdges[i] % 2 == 1:
                oddNumEdges += 1
        #if oddNumEdges is greater than 2 then no Eulerian path exists
        #if oddNumEdges is 0 than it is an Eulerian cycle but not a Eulerian path
        if oddNumEdges > 2 or oddNumEdges == 0:
            return False
        else:
            return True
